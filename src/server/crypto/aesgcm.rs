...
pub fn decipher(encrypted_message: EncMessage, my_secret: StaticSecret, opp_public: PublicKey) -> Result<Vec<u8>> {
    let shared_secret = derive_shared_secret(my_secret, opp_public).as_bytes().to_vec();
    ...
    match decrypt(&decoded_ciphertext, &shared_secret, nonce.as_bytes()) {
        Ok(d) => Ok(d),
        Err(e) => Err(e),
    }
}